...
export function ChatArea({ messages, setSelectedChat, selectedChat, message, setMessage, onSendMessage, recipientName, user, onToggleMobileSidebar }: MessageAreaProps) {
        ...
        return (
            <div
                className={`px-4 py-3 rounded-2xl shadow-lg transition-all duration-200 hover:shadow-xl ${isUser
                    ? "bg-primary text-primary-foreground rounded-br-md max-w-[320px] lg:max-w-[450px] shadow-primary/20"
                    : "bg-card text-card-foreground border border-border rounded-bl-md max-w-[320px] lg:max-w-[450px] shadow-card/20"
                    }`}
            >
    ...
    if (!messages || messages.length === 0) {
        return (
            <div className="w-full h-full flex flex-col bg-gradient-to-br from-muted/5 via-background to-muted/10 relative">
                {/* Background pattern */}
    ...
    return (
        <div
            ref={containerRef}
            className="w-full h-full flex flex-col bg-gradient-to-br from-muted/5 via-background to-muted/10 relative"
        >
            {/* Header Area */}
            {user && (
                <HeaderArea
                                ...
                                <div key={message.id} className={`${isNewSender ? "mt-6" : ""}`}>
                                    <div className={`flex w-full ${isUser ? "justify-end" : "justify-start"} group`}>
                                        ...
                                        <div className={`${isUser ? "ml-auto" : ""} flex flex-col min-w-0`}>
                                            ...
                                            {isLastInGroup && (
                                                <div
                                                    className={`flex items-center gap-2 mt-2 px-3 transition-all duration-300 opacity-0 group-hover:opacity-100 ${isUser ? "justify-end" : "justify-start"
                                                        }`}
                                                >
                                                    ...
                                                    {isUser && (
                                                        <div className="flex items-center gap-1.5">
                                                            {message.isRead ? (
                                                                <CheckCheck className="w-3.5 h-3.5 text-primary animate-pulse" />
                                                            ) : (
